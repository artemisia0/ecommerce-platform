...
export default function InteractiveProductPreviewCard({ product }:
{
	...
	return (
		<Link className="m-0 p-0" href={"/product/" + product.name.replaceAll(" ", "_")}>
			<div className="flex flex-col gap-3 justify-center items-center align-center bg-white/50 p-3 border border-white shadow shadow-lg w-full max-w-64 h-72 backdrop-blur-sm hover:shadow-inner hover:backdrop-blur hover:bg-blue-900/20" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
				<ProductPreviewImage ref={imageRef} preview="/pants.png" extraPreview="/pants2.png" />
